...
impl Config {
    pub fn apply_env_overrides(mut self) -> Self {
        if let Ok(v) = std::env::var("ASR_ONNX_PROFILE_MIN_S") {
            if let Ok(n) = v.parse::<usize>() { self.min_duration_s = n.max(1); }
        }
        let max_env = std::env::var("ASR_ONNX_PROFILE_MAX_S").or_else(|_| std::env::var("ASR_ONNX_PROFILE_SECONDS"));
        if let Ok(v) = max_env {
            if let Ok(n) = v.parse::<usize>() { self.max_duration_s = n.max(1); }
        }
        if self.max_duration_s < self.min_duration_s { std::mem::swap(&mut self.min_duration_s, &mut self.max_duration_s); }
        self.min_duration_s = self.min_duration_s.max(1);
        self.max_duration_s = self.max_duration_s.max(self.min_duration_s);
        self.opt_duration_s = (((self.min_duration_s + self.max_duration_s) as f32) / 2.0).round() as usize;
        self
    }
    pub fn with_num_sessions(mut self, v: usize) -> Self { self.num_sessions = v; self }
    pub fn min_seq_len(&self) -> usize { (self.sample_rate * self.min_duration_s - self.window) / self.hop + 1 }
    pub fn opt_seq_len(&self) -> usize { (self.sample_rate * self.opt_duration_s - self.window) / self.hop + 1 }
    pub fn max_seq_len(&self) -> usize { (self.sample_rate * self.max_duration_s - self.window) / self.hop + 1 }
    pub fn hop(&self) -> usize { self.hop }
    pub fn sample_rate(&self) -> usize { self.sample_rate }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum VocabStyle { WordPiece, SentencePiece, Gpt2Bpe, CharCtc }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TimestampMode { Run, Peak }
...
impl Default for DecodeOptions {
    fn default() -> Self {
        Self { timestamp_mode: TimestampMode::Run, blank_merge_threshold_frames: 0, filter_specials: true }
    }
}
...
impl VocabDecoder {
    pub fn new<P: AsRef<Path>>(vocab_path: P, blank_idx: usize) -> Result<Self> {
        ...
        let mut labels: Vec<String> = txt.lines().map(|l| l.trim_end_matches('\r').to_owned()).collect();
        if blank_idx > labels.len() { bail!("blank_idx {} out of range for vocab len {}", blank_idx, labels.len()); }
        if blank_idx == labels.len() { labels.push(String::new()); } else { labels[blank_idx] = String::new(); }
        let style = Self::detect_style(&labels);
        let specials = Self::default_specials();
        Ok(Self { labels: Arc::new(labels), blank_idx, style, specials })
    }

    pub fn new_from_tokens_file<P: AsRef<Path>>(tokens_path: P) -> Result<Self> {
        let txt = fs::read_to_string(tokens_path)?;
        let tokens: Vec<String> = txt.lines().map(|line| line.split_whitespace().next().unwrap_or("").to_string()).collect();
        let blank_idx = tokens.len() - 1;
        let mut labels = tokens;
        if blank_idx < labels.len() { labels[blank_idx] = String::new(); }
        let style = Self::detect_style(&labels);
    ...
    pub fn vocab_len(&self) -> usize { self.labels.len() }
    pub fn blank(&self) -> usize { self.blank_idx }

    fn detect_style(labels: &[String]) -> VocabStyle {
        if labels.iter().any(|s| s.starts_with("##")) { VocabStyle::WordPiece }
        else if labels.iter().any(|s| s.chars().next() == Some('▁')) { VocabStyle::SentencePiece }
        else if labels.iter().any(|s| s.starts_with('Ġ')) { VocabStyle::Gpt2Bpe }
        else { VocabStyle::CharCtc }
    }

    fn default_specials() -> HashSet<String> {
        ["<unk>", "<pad>", "<blank>", "<ctc_blank>", "<blk>"].iter().map(|s| s.to_string()).collect()
    }
    ...
    fn normalize_piece<'a>(&self, s: &'a str) -> (&'a str, bool, bool) {
        match self.style {
            VocabStyle::WordPiece => (s.strip_prefix("##").unwrap_or(s), !s.starts_with("##"), s.starts_with("##")),
            VocabStyle::SentencePiece => { let start = s.starts_with('▁'); (s.trim_start_matches('▁'), start, !start) }
            VocabStyle::Gpt2Bpe => { let start = s.starts_with('Ġ'); (s.trim_start_matches('Ġ'), start, !start) }
            VocabStyle::CharCtc => (s, false, false),
        }
    }

    fn is_closing_punct(s: &str) -> bool {
        matches!(s, "." | "," | "!" | "?" | ";" | ":" | "%" | ")" | "]" | "}" | "'") || s == "'" || s == "\"" || s == "\u{201D}" || s == "\u{2019}"
    }

    fn is_apostrophe(s: &str) -> bool { s == "'" || s == "'" }
    ...
    pub fn decode_ids_with_timestamps(
        ...
        if ids.is_empty() { return (String::new(), Vec::new()); }
        let ms_per_step = (hop as f32 / sample_rate as f32) * 1000.0 * (in_frames as f32 / enc_steps.max(1) as f32);
        let mut pieces: Vec<(String, u32, u32)> = Vec::with_capacity(ids.len());
        for (i, &id) in ids.iter().enumerate() {
            if id == self.blank_idx { continue; }
            let tok = &self.labels[id];
            if self.is_special_token(tok) { continue; }
            let s = (emit_t[i] as f32 * ms_per_step).round() as u32;
            let e = ((emit_t.get(i + 1).copied().unwrap_or(emit_t[i]) as f32) * ms_per_step).round() as u32;
        ...
        for (tok, s, e) in pieces {
            ...
            if Self::is_closing_punct(piece) || Self::is_apostrophe(piece) {
                if first { curr.push_str(piece); w_s = s; w_e = e; first = false; } else { curr.push_str(piece); w_e = e; }
                continue;
            }
            if matches!(self.style, VocabStyle::WordPiece | VocabStyle::SentencePiece | VocabStyle::Gpt2Bpe) && starts_word {
                if !first && !curr.is_empty() {
                    words.push((curr.clone(), w_s, w_e));
                    curr.clear();
                }
                ...
                if first { w_s = s; first = false; }
                curr.push_str(piece);
                w_e = e;
            }
        }
        if !first && !curr.is_empty() { words.push((curr.clone(), w_s, w_e)); }
...
pub fn unmarshal(bytes: Bytes) -> TranscriptionResult {
    decode_from_slice(&bytes, config::standard()).expect("deserialize").0
}

trait AsrEngine: Send {
    fn run(&mut self, features: &Array2<f32>, cfg: &Config, vocab: &VocabDecoder) -> Result<(String, Vec<(String, u32, u32)>, f64)>;
}
...
impl TdtEngine {
    ...
    fn encoder_infer(&mut self, features: &Array2<f32>) -> Result<Array3<f32>> {
        ...
        let enc = outputs.get("outputs").ok_or_else(|| anyhow::anyhow!("missing encoder output"))?;
    ...
    fn joint_enc_infer(&mut self, encoder_output: &Array3<f32>) -> Result<ArrayD<f32>> {
        ...
        let out = outputs.get("output").ok_or_else(|| anyhow::anyhow!("missing joint_enc output"))?;
    ...
    fn decoder_infer(
        ...
        let out = outputs.get("outputs").ok_or_else(|| anyhow::anyhow!("missing decoder outputs"))?;
        let mut dec = out.try_extract_array::<f32>()?;
        dec = dec.permuted_axes(IxDyn(&[0, 2, 1]));
        let s0 = outputs.get("states").ok_or_else(|| anyhow::anyhow!("missing decoder state0"))?;
        let s1 = outputs.get("162").ok_or_else(|| anyhow::anyhow!("missing decoder state1"))?;
        Ok((dec.to_owned(), s0.try_extract_array::<f32>()?.to_owned(), s1.try_extract_array::<f32>()?.to_owned()))
    }

    fn joint_pred_infer(&mut self, decoder_output: &ArrayD<f32>) -> Result<ArrayD<f32>> {
        let inputs = ort::inputs!["onnx::MatMul_0" => OrtTensor::from_array(decoder_output.clone())?];
        let outputs = self.joint_pred.run(inputs)?;
        let out = outputs.get("5").ok_or_else(|| anyhow::anyhow!("missing joint_pred output"))?;
        Ok(out.try_extract_array::<f32>()?.to_owned())
    }
    ...
    fn joint_net_infer(
        ...
        let out = outputs.get("6").ok_or_else(|| anyhow::anyhow!("missing joint_net output"))?;
...
impl AsrEngine for TdtEngine {
    fn run(&mut self, features: &Array2<f32>, cfg: &Config, vocab: &VocabDecoder) -> Result<(String, Vec<(String, u32, u32)>, f64)> {
        ...
        while active {
            let (mut decoder_output, state0_next, state1_next) = self.decoder_infer(label, &state0, &state1)?;
            decoder_output = self.joint_pred_infer(&decoder_output)?;
            let f_slice = encoder_output_projected.slice(s![0, safe_time_index, ..]).to_owned().into_dyn();
            ...
            for (i, &v) in dur_slice.iter().enumerate() {
                if v > dur_val { dur_val = v; dur_idx = i; }
            }
            ...
            if time_index >= last_timestamps { active = false; }
            if active && (label as usize) == self.blank_id { advance = true; }
            ...
            while advance {
                ...
                let mut duration = dur_idx as i32;
                if (label as usize) == self.blank_id && duration == 0 { duration = 1; }
                time_index += duration as usize;
                safe_time_index = time_index.min(last_timestamps);
                if time_index >= last_timestamps { active = false; }
                if active && (label as usize) == self.blank_id { advance = true; } else { advance = false; }
            }
        ...
        let (text, words) = vocab.decode_ids_with_timestamps(&ids, &emits, t_enc, cols, cfg.hop, cfg.sample_rate);
...
impl SessionPool {
    pub fn new<P: AsRef<Path>>(model_dir: P, vocab: P, device_id: usize, config: Config) -> Result<Self> {
        ...
        for s in 0..config.num_sessions {
            ...
            thread::spawn(move || {
                let result: Result<()> = (|| {
                    ...
                    if !model_path.is_dir() { bail!("TDT requires a model directory with component ONNX files"); }
                    ...
                    if !CUDAExecutionProvider::default().is_available().unwrap_or(false) {
                        panic!("CUDA Execution Provider not available: TDT models require CUDA-enabled ONNX Runtime");
                    }
        ...
        let enc_path = if enc_fp16.exists() { enc_fp16.clone() } else if enc_fp32.exists() { enc_fp32.clone() } else { enc_int8.clone() };
        let dec_path = if dec_fp16.exists() { dec_fp16.clone() } else if dec_fp32.exists() { dec_fp32.clone() } else { dec_int8.clone() };
        let joint_enc_path = if joint_enc_fp16.exists() { joint_enc_fp16.clone() } else if joint_enc_fp32.exists() { joint_enc_fp32.clone() } else { joint_enc_int8.clone() };
        let joint_pred_path = if joint_pred_fp16.exists() { joint_pred_fp16.clone() } else if joint_pred_fp32.exists() { joint_pred_fp32.clone() } else { joint_pred_int8.clone() };
        let joint_net_path = if joint_net_fp16.exists() { joint_net_fp16.clone() } else if joint_net_fp32.exists() { joint_net_fp32.clone() } else { joint_net_int8.clone() };

        if !enc_path.exists() || !dec_path.exists() || !joint_enc_path.exists() || !joint_pred_path.exists() || !joint_net_path.exists() {
            bail!("Missing required TDT component(s) in {}", model_path.display());
        }
                    ...
                    info!(device = did, session = sid, vocab_len, blank_idx = vocab_decoder.blank(), "vocab loaded");

                    let cuda = CUDAExecutionProvider::default().with_device_id(did as i32).build();
                    let cpu = CPUExecutionProvider::default().build();

                    let enc = match Session::builder()?
                        .with_optimization_level(GraphOptimizationLevel::Level3)?
                        .with_log_level(LogLevel::Info)?
                        .with_execution_providers([cuda.clone()])?
                        .with_intra_threads(1)?
                        .commit_from_file(&enc_path)
                    {
                        Ok(s) => s,
                        Err(e_cuda) => {
                            warn!(device = did, session = sid, ?e_cuda, "CUDA encoder failed, falling back to CPU");
                            Session::builder()?
                                .with_optimization_level(GraphOptimizationLevel::Level3)?
                                .with_log_level(LogLevel::Info)?
                                .with_execution_providers([cpu.clone()])?
                                .with_intra_threads(1)?
                                .commit_from_file(&enc_path)?
                        }
                    };

                    let dec = match Session::builder()?
                        .with_optimization_level(GraphOptimizationLevel::Level3)?
                        .with_log_level(LogLevel::Info)?
                        .with_execution_providers([cuda.clone()])?
                        .with_intra_threads(1)?
                    ...
                    let jenc = match Session::builder()?
                        .with_optimization_level(GraphOptimizationLevel::Level3)?
                        .with_log_level(LogLevel::Info)?
                        .with_execution_providers([cuda.clone()])?
                        .with_intra_threads(1)?
                        .commit_from_file(&joint_enc_path)
                    {
                        Ok(s) => s,
                        Err(_) => Session::builder()?
                            .with_optimization_level(GraphOptimizationLevel::Level3)?
                    ...
                    let jpred = match Session::builder()?
                        .with_optimization_level(GraphOptimizationLevel::Level3)?
                        .with_log_level(LogLevel::Info)?
                        .with_execution_providers([cuda.clone()])?
                        .with_intra_threads(1)?
                        .commit_from_file(&joint_pred_path)
                    {
                        Ok(s) => s,
                        Err(_) => Session::builder()?
                            .with_optimization_level(GraphOptimizationLevel::Level3)?
                    ...
                    let jnet = match Session::builder()?
                        .with_optimization_level(GraphOptimizationLevel::Level3)?
                        .with_log_level(LogLevel::Info)?
                        .with_execution_providers([cuda.clone()])?
                        .with_intra_threads(1)?
                        .commit_from_file(&joint_net_path)
                    {
                        Ok(s) => s,
                        Err(_) => Session::builder()?
                            .with_optimization_level(GraphOptimizationLevel::Level3)?
                    ...
                    let mut engine = TdtEngine::new(enc, dec, jpred, jenc, jnet, vocab_decoder.blank(), vocab_len);
                    let mut decoder_opt: Option<VocabDecoder> = None;
                    let _ = ready_tx_clone.send(());

                    loop {
                        let job = match rx.recv() { Ok(j) => j, Err(_) => break };
                        debug!(device = did, session = sid, job_id = job.job_id, seq = job.meta.seq, rows = job.features.dim().0, cols = job.features.dim().1, "infer start");
                        let start = Instant::now();
                        if decoder_opt.is_none() {
                            decoder_opt = Some(if tokens_path.exists() { VocabDecoder::new_from_tokens_file(&tokens_path)? } else { VocabDecoder::new(&v, c.blank_idx)? });
                            let d = decoder_opt.as_ref().unwrap();
                            info!(device=did, session=sid, vocab_len=d.vocab_len(), blank_idx=d.blank(), "decoder init");
                        }
                        let d = decoder_opt.as_mut().unwrap();
                        let (text, words, gpu_time_ms) = engine.run(&job.features, &c, d)?;
                        if text.trim().is_empty() {
                            warn!(device = did, session = sid, job_id = job.job_id, seq = job.meta.seq, words = words.len(), "Empty ASR transcription text");
                        }
                        let task_time_ms = start.elapsed().as_secs_f64() * 1e3;
                        debug!(device = did, session = sid, job_id = job.job_id, seq = job.meta.seq, gpu_ms = gpu_time_ms, task_ms = task_time_ms, text_len = text.len(), words = words.len(), "infer done");
                ...
                if let Err(e) = result { error!(device = did, session = sid, ?e, "session thread error"); }
            });
        }

        Ok(Self { submit_tx: tx_infer, result_rx: rx_res, ready_rx })
    }

    pub fn submit(&self, job: TranscriptionJob) -> Result<()> {
        debug!(job_id = job.job_id, seq = job.meta.seq, rows = job.features.dim().0, cols = job.features.dim().1, "pool submit");
        self.submit_tx.send(job)?;
        Ok(())
    }

    pub fn result_rx(&self) -> &Receiver<TranscriptionResult> { &self.result_rx }
    pub fn ready_rx(&self) -> &Receiver<()> { &self.ready_rx }
}
...
impl TranscriberPool {
    pub fn new<P: AsRef<Path>>(model: P, vocab: P, device_ids: &[usize], config: Config) -> Result<Self> {
        ...
        for &id in &effective_device_ids {
            ...
            let ready_tx_clone = ready_tx.clone();
            thread::spawn(move || { for r in rx { let _ = tx.send(r); } });
            thread::spawn(move || { for _ in pool_ready_rx { let _ = ready_tx_clone.send(()); } });
        ...
        Ok(Self { job_counter, submit_tx, result_rx, ready_rx })
    }
    ...
    pub fn submit(&self, name: String, features: Array2<f32>, meta: JobMeta) -> Result<u64> {
        ...
        let job = TranscriptionJob { job_id: id, name, features: Arc::new(features), meta };
    ...
    pub fn result_rx(&self) -> &Receiver<TranscriptionResult> { &self.result_rx }
    pub fn ready(&self) -> &Receiver<()> { &self.ready_rx }
}
